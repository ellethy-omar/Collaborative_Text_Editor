...
public class Session {
    ...
    public synchronized String addUser(String username) {
        if (users.size() >= 4) throw new IllegalStateException("Session is full");
        String assigned = username;
        if (assigned == null || assigned.isBlank()) {
            if (availableNames.isEmpty()) throw new IllegalStateException("No available user slots");
            assigned = availableNames.poll();
        }
        if (!users.add(assigned)) throw new IllegalArgumentException("Username already in session: " + assigned);
        return assigned;
    }

    public synchronized boolean removeUser(String username) {
        boolean removed = users.remove(username);
        if (removed) availableNames.offer(username);
        return removed;
    }
    ...
    public synchronized String getSessionText() { return sessionText; }
